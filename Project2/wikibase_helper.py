...
from wikidataintegrator import wdi_config
...
class WikibaseHelper:
    ...
    def get_pid(self, uri):
        ...
        if (self.sparql_endpoint_url == wdi_config.config['SPARQL_ENDPOINT_URL'] and
                (uri.startswith("P") or uri.startswith(wdi_config.config['CONCEPT_BASE_URI']))):
    ...
    def get_qid(self, uri):
        ...
        if (self.sparql_endpoint_url == wdi_config.config['SPARQL_ENDPOINT_URL'] and
                (uri.startswith("Q") or uri.startswith(wdi_config.config['CONCEPT_BASE_URI']))):
