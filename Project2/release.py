...
class Release(object):
    ...
    def make_statements(self):
        ...
        helper = self.helper
        # instance of edition
        s.append(wdi_core.WDItemID(helper.get_qid(ITEMS['version, edition, or translation']),
                                   helper.get_pid(PROPS['instance of'])))
        # edition or translation of
        s.append(wdi_core.WDItemID(self.edition_of_qid, helper.get_pid(PROPS['edition or translation of'])))
        # edition number
        s.append(wdi_core.WDString(self.edition, helper.get_pid(PROPS['edition number'])))

        if self.archive_url:
            s.append(wdi_core.WDUrl(self.archive_url, helper.get_pid(PROPS['archive URL'])))

        if self.pub_date:
            s.append(wdi_core.WDTime(self.pub_date, helper.get_pid(PROPS['publication date']),
    ...
    def get_or_create(self, login=None):
        ...
        helper = self.helper
        edition_dict = id_mapper(helper.get_pid(PROPS['edition number']),
                                 ((helper.get_pid(PROPS['edition or translation of']), self.edition_of_qid),
                                  (helper.get_pid(PROPS['instance of']),
                                   helper.get_qid(ITEMS['version, edition, or translation']))),
